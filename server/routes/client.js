...
//   qString += ' and c.client_id = ?';
  ...
  let qString = 'SELECT * ';
  qString +=    'FROM transaction ';
  qString +=    'LEFT JOIN trans_reltn ON transaction.trans_id=trans_reltn.trans_id ';
  qString +=    'LEFT JOIN grant_data ON trans_reltn.grant_id=grant_data.grant_id ';
  qString +=    'WHERE transaction.client_id = ?';
